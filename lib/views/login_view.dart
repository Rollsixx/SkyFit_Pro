...
class _LoginViewState extends State<LoginView> {
  ...
  Future<void> _login() async {
    ...
    if (!mounted) return;
    if (ok) _goTodos();
    else _snack(auth.error ?? 'Login failed');
  }
  ...
  Future<void> _googleLogin() async {
    ...
    if (ok) {
      // Google auth succeeded — show OTP dialog
      _otpCtrl.clear();
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) {
          return AlertDialog(
            ...
            title: Row(
              children: [
                Icon(Icons.mark_email_read_outlined, color: cs.secondary),
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                const Text(
                  'Check your inbox (or debug console).',
  ...
  Future<void> _bioUnlock() async {
    ...
    if (!mounted) return;
    if (ok) _goTodos();
    else _snack(auth.error ?? 'Biometric unlock failed');
  }
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          Positioned(
            top: -80, right: -60,
            child: _Blob(color: cs.primary.withOpacity(isDark ? 0.25 : 0.12), size: 260),
          ),
          Positioned(
            bottom: -60, left: -40,
            child: _Blob(color: cs.secondary.withOpacity(isDark ? 0.20 : 0.10), size: 200),
          ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (auth.biometricsChecked && !auth.biometricsAvailable) ...[
                        ...
                        Text(
                          'Tip: Enroll a fingerprint and enable screen lock to use biometrics.',
                          textAlign: TextAlign.center,
                          style: TextStyle(
                            color: isDark ? Colors.white38 : Colors.black38,
                            fontSize: 12,
